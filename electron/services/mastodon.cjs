...
async function fetchMastodonImageLimits({ instanceUrl, accessToken, limitsCache, forceRefresh, onDiscoveredLimits }) {
...
async function uploadMedia({ instanceUrl, accessToken, photoPath, description, onProgress, imageResizeOptions, instanceLimitsCache, onDiscoveredLimits }) {
  ...
  while (true) {
    const limits = await fetchMastodonImageLimits({
      instanceUrl,
      accessToken,
